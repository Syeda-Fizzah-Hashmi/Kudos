...
public class GameEnds extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        if(isResult()==true && score>0){
            ...
            AudioClip win = new AudioClip(new File("src/Audio/mixkit-game-level-completed-2059.wav").toURI().toString());

            //setting up the background
            FileInputStream input1 = new FileInputStream("src/images/debut-space-dribbble.png");
            ...
            AudioClip lose = new AudioClip(new File("src/Audio/mixkit-game-over-trombone-1940.wav").toURI().toString());

            //setting up the background
            FileInputStream input2 = new FileInputStream("src/images/debut-space-dribbble.png");
            ...
            AudioClip newRecord = new AudioClip(new File("src/Audio/mixkit-game-bonus-reached-2065.wav").toURI().toString());

            //setting up the background
            FileInputStream input3 = new FileInputStream("src/images/debut-space-dribbble.png");
    ...
    public VBox caseWin(GameButtons playAgain, GameButtons menu){
        ...
        user.setSpaceRoute_score(score);
        scoreWriter(user,score);
    ...
    public void scoreWriter(MainUser user, int score){
        try {
            ArrayList<MainUser> userList = new ArrayList<>();
            // If file is Empty
            if ((new File("SpaceRouterData.ser")).length() == 0) {
                System.out.println("Space router score:"+ score);
                user.setSpaceRoute_score(score);
                userList.add(user);
                ObjectOutputStream writeFile;
                writeFile = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream("SpaceRouterData.ser")));
            ...
            // If file is not Empty
                ...
                for (int i = 0; i < userList.size(); i++) {
                    if (user.getUserName().equals(userList.get(i).getUserName())) {
                        userList.get(i).setSpaceRoute_score(score);
                        ObjectOutputStream writeFile = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream("LogicRaceData.ser")));
                        writeFile.writeObject(userList);
                        writeFile.close();
                        flag = 1;
                        break;
